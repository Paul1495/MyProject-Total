...
import { orderDetails, targets, outputs } from "../report_509/data_STT509.js";
...
$.each(listLine, (i, line) => {
  ...
  $.each(orders, (y, order) => {
    ...
    const target = targets.find(
      (target) => target.Zdcode === order && target.Workline === line
    );
    //Create a empty variable to push all data after processing
    const combine = {
      ...orderDetail,
...
const grid = $("#grid")
  .dxDataGrid({
    ...VIEW_GRID_OPTION,
    dataSource: data,
    columns: [...columns_data, ...setupAllTable.map((x) => x.column)],
    showBorders: true,
    wordWrapEnabled: true,
    summary: {
      groupItems: [...setupAllTable.map((x) => x.summary).flat()],
    },
  })
  .dxDataGrid("instance");
