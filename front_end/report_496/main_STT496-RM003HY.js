import { orderDetails, targets, outputs } from "../report_496/data_STT496.js";
...
$.each(listLine, (i, line) => {
  //Lặp qua từng phần tử trong biến listLine
  ...
  $.each(orders, (y, order) => {
    //Lặp qua từng phần tử trong mảng chứa dữ liệu Zdcode duy nhất của từng chuyền
    ...
    const target = targets.find(
      (z) => z.Zdcode === order && z.Workline === line
    ); // Set điều kiện để lấy targetQuantity
    const combine = {
      ...orderDetail,
      Workline: line,
      Zdcode: order,
    ...
    $.each(orderData, (o, data) => {
      //Tạo vòng lặp qua từng phần từ của OrderData khi đã gom về từng giá trị duy nhất, sau đó gộp chung lại
    ...
    columns: [
      ...
      },
      {
        ...
        calculateCellValue: (rowData) => {
          ...
          rowData[lackDataField] =
            rowData["TargetQuantity"] - rowData[acculmulateDataField];
...
const setupTable = [
  {
    ...
    calculateCellValue: (rowData) => {
      rowData["697To698"] =
        rowData["697Accumulated"] - rowData["698Accumulated"];
  ...
  },
  {
    ...
    calculateCellValue: (rowData) => {
      rowData["698To700"] =
        rowData["698Accumulated"] - rowData["700Accumulated"];
  ...
  },
  {
    ...
    calculateCellValue: (rowData) => {
      rowData["697To700"] =
        rowData["697Accumulated"] - rowData["700Accumulated"];
