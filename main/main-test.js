    ...
    let object = infor;
        StoreIOs.forEach((store) => {
            if(infor.ColorNo === store.ColorNo && infor.MO === store.Zdcode && infor.Size === store.CM) {
                ...
                object[`${store.StoreNo}_IN_LACK`] = infor.Quantity - object[`${store.StoreNo}_IN`]
                object[`${store.StoreNo}_OUT_LACK`] = infor.Quantity - object[`${store.StoreNo}_OUT`]
            }
        });
...
// combined_data.forEach((total) => {
//     let analyst = total;
//     analyst[`DCUT_IN_LACK`] = total.Quantity - total.DCUT_IN;
//     analyst[`DCUT_OUT_LACK`] = total.Quantity - total.DCUT_OUT;
//     analyst[`DTSW1FOAM_IN_LACK`] = total.Quantity - total.DTSW1FOAM_IN;
//     dataload.push(analyst);     
// });
